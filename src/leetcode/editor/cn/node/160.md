...
## 思路
这题的思路比较直接，我们设第一个链表除相交部分的长度为a，第二个链表除相交部分的长度为b，不失一般性，我们假设a >= b，那么我们只需要知道a - b的值。随后设nodeA为第一个链表的其起始节点，nodeB为第二个链表的起始节点，那么我们让nodeA先走a-b步，后续让nodeA和nodeB同时往后走，等到第一个nodeA == nodeB，返回该node即可。
